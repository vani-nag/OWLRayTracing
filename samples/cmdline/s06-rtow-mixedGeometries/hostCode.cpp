...
int main(int ac, char **argv)
{
	...
	{
		for(int i = 0; i < knn; i++)
			neighbors.push_back(Neigh{-1,FLOAT_MAX, knn, 0});
	}
	...
	//Cheeck for sparse points after 2nd round
	if(numRounds == 3)
	...
	std::cout<<"Points without k neighbors = "<<count<<'\n';
	...
	for(int j=0; j<Spheres.size(); j++)
	{
		...
		if(fb[j*knn].numNeighbors > 0 && fb[j*knn].sparse == 0)
		{
			foundKNN = 0;
			radius *= 2;
	...
	*/
	...
	while(!foundKNN) //&& (stop==0 | stop==1))
	{
		...
		for(int j=0; j<Spheres.size(); j++)
		{
			...
			if(fb[j*knn].numNeighbors > 0)
			{
				//std::cout<<"numNeighs = "<<fb[j*knn].numNeighbors<<'\n';
				count++;
			}
				
			
		}
		std::cout<<"Sparse points = "<<count<<'\n';
	...
	//Account time for sparse points also
