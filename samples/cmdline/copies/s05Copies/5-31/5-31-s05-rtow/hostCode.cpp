...
int main(int ac, char **av)
{
  ...
  // ##################################################################
	...
	{
		Spheres.push_back(Sphere{vec3f(vect.at(i),vect.at(i+1),-1),radius,c,0});
		c++;
	}
  ...
  // ##################################################################
	...
	{
			//Select ray origin
			vec3f rayOrigin = Spheres.at(0).center;
			cout<<"Spheres: "<<"\t\n";
			...
			//for(int i = 0; i < Spheres.size()+numRemoved; i++)
				flag = 0;
				for (auto i = Spheres.begin(); i != Spheres.end(); ++i)
				{
		      if (fb[i -> index] == 1) 
					{
							cout<<"Erasing "<< i->index<<'\n';
		          Spheres.erase(i);
		          i--;
							flag = 1;
        	}
